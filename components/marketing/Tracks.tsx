  ...
  return (
    <section className="section-wrap" id="tracks">
      ...
      <h2 className="font-serif text-[clamp(2rem,4vw,3rem)] text-[var(--ink)] tracking-[-0.5px] leading-[1.1] mb-3">
        Two tracks.<br /><em className="text-[var(--accent)] not-italic">One destination.</em>
      </h2>
      ...
      <div
        className="grid md:grid-cols-2 gap-px bg-[var(--border)] border border-[var(--border)] mb-12"
        role="list"
        aria-label="Learning tracks"
      >
        {/* Track A */}
        <div
          className="relative bg-[var(--surface)] p-10 transition-all duration-200 hover:bg-[var(--surface2)] hover:shadow-[inset_0_0_40px_rgba(129,140,248,0.04)] overflow-hidden"
          role="listitem"
        >
          <span className="absolute top-5 right-5 font-mono text-[9px] tracking-[0.15em] uppercase text-[var(--muted)] opacity-40" aria-hidden="true">
            Track A
          </span>
          <span className="text-[2.5rem] mb-5 block" aria-hidden="true">🛡️</span>
          <h3 className="font-serif text-[1.5rem] text-[var(--ink)] mb-2 tracking-[-0.3px]">The Security Pro Track</h3>
          <div className="font-mono text-[10px] tracking-[0.1em] uppercase text-[var(--muted)] mb-4">
            Coming from: SOC / Pentest / Blue Team / Compliance
          </div>
          <p className="text-[13px] text-[var(--ink2)] leading-[1.7] mb-6">
          ...
          <div className="flex flex-wrap gap-1.5 mb-6" aria-label="Skills">
            {['✓ Threat Modeling', '✓ SIEM / Detection', '✓ Vuln Management'].map((s) => (
              <span key={s} className="font-mono text-[10px] tracking-[0.05em] px-2.5 py-1 bg-[var(--bg2)] border border-[var(--border)] text-[var(--ink2)]">
                {s}
              </span>
            ))}
            {['→ ML Fundamentals', '→ Prompt Injection', '→ AI Audit Frameworks', '→ LLM Red Teaming'].map((s) => (
              <span key={s} className="font-mono text-[10px] tracking-[0.05em] px-2.5 py-1 bg-[rgba(129,140,248,0.08)] border border-[rgba(129,140,248,0.2)] text-[var(--accent)]">
                {s}
              </span>
            ))}
          </div>
          <div className="font-mono text-[11px] text-[var(--muted)] flex items-center gap-2 mb-6">
            Estimated timeline: <strong className="text-[var(--accent)]">3–5 months</strong> to job-ready
          </div>
          <Link
        ...
        {/* Track B */}
        <div
          className="relative bg-[var(--surface)] p-10 transition-all duration-200 hover:bg-[var(--surface2)] hover:shadow-[inset_0_0_40px_rgba(34,211,238,0.03)] overflow-hidden"
          role="listitem"
        >
          <span className="absolute top-5 right-5 font-mono text-[9px] tracking-[0.15em] uppercase text-[var(--muted)] opacity-40" aria-hidden="true">
            Track B
          ...
          <h3 className="font-serif text-[1.5rem] text-[var(--ink)] mb-2 tracking-[-0.3px]">The Career Changer Track</h3>
          <div className="font-mono text-[10px] tracking-[0.1em] uppercase text-[var(--muted)] mb-4">
          ...
          <div className="flex flex-wrap gap-1.5 mb-6" aria-label="Skills">
            ...
            {['→ Security Fundamentals', '→ How AI Systems Work', '→ Risk Frameworks', '→ AI Governance & Audit', '→ Your Domain + AI Security'].map((s) => (
              <span key={s} className="font-mono text-[10px] tracking-[0.05em] px-2.5 py-1 bg-[rgba(34,211,238,0.06)] border border-[rgba(34,211,238,0.15)] text-[var(--accent2)]">
                {s}
              </span>
            ))}
          </div>
          <div className="font-mono text-[11px] text-[var(--muted)] flex items-center gap-2 mb-6">
            Estimated timeline: <strong className="text-[var(--accent2)]">6–9 months</strong> to job-ready
          </div>
          <Link
            href="/roadmap?track=career-changer"
            className="inline-flex items-center gap-2 text-[13px] font-semibold text-[var(--accent2)] border-b border-[var(--accent2)] pb-0.5 transition-all duration-200 hover:gap-3"
